...
function showcity(idx) {
 ...
 if ( idx ) {
  ...
  for( var i = 1; i < citys.length; i++ ) {
   e = document.createElement( "option" );
   e.setAttribute( "value", pc[idx][0]+"-"+citys[i] );
   e.innerHTML = citys[i];
   c.appendChild(e);
  }
 }
...
<form action="/oa/expense/newaddcostdetails.action" method="post"
	  ...
	  <div class="pageFormContent" layoutH="50"> 
	   ...
	   <dl class="nowrap">
	     ...
	     <dl class="nowrap">
	       ...
	       <input name="costDetails.expenseTime" type="text" class="date textInput readonly valid" value="${costDetails.expenseTime}" class="required"/>
	     ...
	     <input name="costDetails.payMoney" type="text"  value="${costDetails.payMoney}" class="required number"/>元
	     </dl>
	      ...
	      <input name="costDetails.payTime" type="text" class="date textInput readonly valid" value="${costDetails.payTime}" class="required"/>
	      <a class="inputDateButton" href="javascript:;"></a>
	      </dl> 
