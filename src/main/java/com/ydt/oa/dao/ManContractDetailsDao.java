 ...
 */
@Repository
public class ManContractDetailsDao extends BaseDaoHibernate<ManContractDetails>{
     ...
     public Pagination list(int currPage,int pageSize){		
        ...
        buf.append("from ManContractDetails");
        return pageQuery(buf.toString(), params.toArray(),currPage, pageSize);
	}
	
     
     @SuppressWarnings("unchecked")
	public List<ManContractDetails> list(Contract contract){		
			
	        StringBuilder buf=new StringBuilder();
	        List<Object> params=new ArrayList<Object>();
	        buf.append("from ManContractDetails where contract.id = ?");
	        params.add(contract.getId());
	        return  (List<ManContractDetails>) queryList(buf.toString(), params.toArray());
		}
