		...
		itemDetail: function(){
			return this.each(function(){
				...
				if (addButTxt) {
					...
					var $rowNum = $('<input type="hidden" name="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="1"/>').insertBefore($table);
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'hidden':
						...
						break;
					case 'readonly':
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" readonly="readonly" class="'+field.fieldClass+'" '+attrFrag+'/>';
						break;
					default:
